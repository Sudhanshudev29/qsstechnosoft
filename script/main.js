const texts = ["Generative AI", "Blockchain Solutions", "Salesforce Integration", "Cloud Services"];
let currentText = 0;
let currentIndex = 0;
let typingSpeed = 100;
...
function typeWriter() {
  if (currentIndex < texts[currentText].length) {
    textElement.innerHTML += texts[currentText].charAt(currentIndex);
    currentIndex++;
...
function eraseText() {
  if (currentIndex >= 0) {
    textElement.innerHTML = texts[currentText].substring(0, currentIndex);
    currentIndex--;
...
// js code for dorpdown navbar


const servicesNavbar = document.getElementById('services_navbar');
const dropdown = document.querySelector('.dropdown');

servicesNavbar.addEventListener('mouseenter', () => {
  dropdown.style.display = 'block';
});

servicesNavbar.addEventListener('mouseleave', () => {
